...
export const PVal = props => {
  const p = 1 - jStat.chisquare.cdf(props.x, 7);
  console.log(p);
  if (p === '0') {
...
Looking at the S.D. and mean of the variables, for example, number of overtimes hours have its mean at 3.907. This means that a survey participant who with the variable of zero hours has much smaller value of utility function than the others. The same principle applies to other variables, leading that a person with zero vector of variables does no longer mean "neutral" but somehow "characteristic" property of an observation.
